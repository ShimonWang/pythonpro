...
# IEEE论文的 science + ieee 样式，使用标准颜色：
...
# science + hight-vis 风格：high-vis:high-visibility 高可见度
...
# science + notebook 风格用于 Jupyter 笔记本：
...
# Plot different color cycles # 绘制不同的颜色循环

with plt.style.context(["science", "bright"]):  # bright:明亮的
    fig, ax = plt.subplots()
...
with plt.style.context(["science", "vibrant"]):  # vibrant:鲜艳的
    fig, ax = plt.subplots()
    for p in [5, 10, 15, 20, 30, 50, 100]:
        ax.plot(x, model(x, p), label=p)
    ax.legend(title="Order")
...
with plt.style.context(["science", "muted"]):  # muted:柔和的
    fig, ax = plt.subplots()
...
with plt.style.context(["science", "retro"]):  # retro:复古
    fig, ax = plt.subplots()
    for p in [10, 15, 20, 30, 50, 100]:
        ax.plot(x, model(x, p), label=p)
    ax.legend(title="Order")
...
with plt.style.context(["science", "grid"]):  # grid:网格
    fig, ax = plt.subplots()
    for p in [10, 15, 20, 30, 50, 100]:
        ax.plot(x, model(x, p), label=p)
    ax.legend(title="Order")
...
with plt.style.context(["science", "high-contrast"]):  # high-contrast:高对比度
    fig, ax = plt.subplots()
    for p in [10, 20, 50]:
        ax.plot(x, model(x, p), label=p)
    ax.legend(title="Order")
...
with plt.style.context(["science", "light"]):  # light:浅色的
    fig, ax = plt.subplots()
